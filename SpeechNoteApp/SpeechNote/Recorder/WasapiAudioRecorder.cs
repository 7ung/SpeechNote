...
namespace SpeechNote.Recorder
{
    ...
    public class WasapiAudioRecorder
    {
        ...
        public void StartRecording()
        {
            ...
            bool rs = wasapiAudio.StartAudioCapture();
            Debug.WriteLine("StartAudioCapture" + rs.ToString());
            intervalTimer.Start();
        }
        ...
        void intervalTimer_Tick(object sender, EventArgs e)
        {
            ...
            if (wasapiIsEnabled)
            {
                ...
                // Đọc dữ liệu ghi âm lưu thành byte, và trả về số byte ghi được
                int size = wasapiAudio.ReadBytes(out bytes);

                // Nếu có nhận được âm thanh -> size >0 -> kích hoạt sự kiện BufferReady
                if (size > 0)
                {
                    OnBufferReady(bytes);
                }
